...
pub async fn get_wishes(user_id: web::Path<i32>) -> impl Responder {
    ...
    match conn.exec_map(query, (user_id.into_inner(),), |(id, user_id, text, timestamp): (i32, i32, String, NaiveDateTime)| {
        Wish { 
            id, 
            user_id, 
            text, 
        ...
        Ok(result) => {
            //println!("Fetched wishes: {:?}", result);  // Logging fetched data
...
pub async fn add_wish(user_id: web::Path<i32>, new_wish: web::Json<NewWish>) -> impl Responder {
    ...
    match conn.exec_drop(
        ...
        (user_id.into_inner(), new_wish.text.clone(), now_ist.naive_local())
    ) {
